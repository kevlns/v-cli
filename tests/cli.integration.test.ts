...
describe("CLI 集成", () => {
  it("--version 为 0.2.2", () => {
    expect(run(["--version"], newHome()).trim()).toBe("0.2.2");
  });
  ...
  it("doctor --json 输出结构化体检报告（含 officialPlugins，官方依赖已安装）", () => {
    ...
    if (process.platform === "win32") expect(unity.version).toBe("0.1.3");
  });
