...
describe("CLI 集成", () => {
  it("--version 为 0.2.0-beta.4", () => {
    expect(run(["--version"], newHome()).trim()).toBe("0.2.0-beta.4");
  });
  ...
  it("doctor --json 输出结构化体检报告（含 officialPlugins，官方依赖已安装）", () => {
    ...
    expect(xl.version).toBe("1.2.1-beta.5");
    ...
    if (process.platform === "win32") expect(unity.version).toBe("0.1.0-beta.4");
  });
...
describe("CLI 集成：官方插件命令拦截与转发（fixture 注入）", () => {
  ...
  it("注入环境：V_CLI_HOST_VERSION / V_CLI_PLUGIN_API / V_CLI_INVOKED_BY", () => {
    ...
    expect(payload.env).toEqual({
      V_CLI_HOST_VERSION: "0.2.0-beta.4",
      V_CLI_PLUGIN_API: "1",
